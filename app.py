...
def process_excel(uploaded_file):
    ...
    agents_rules = [
        ...
        (r'(أمضاء|امضاء)\s+ند[ىي]',                                                           '004'),
        (r'(أمضاء|امضاء)\s+خلود',                                                             '004'),
        (r'NADA|nada',                                                                         '004'),
        (r'(ا.?م.?ض.?ا.?ء?\s*)ند[يى]\s*مصطف[يى]',                                           '004'),
    ]
    ...
    for index, row in df.iterrows():
        ...
        if confirmation_agent is None and any(
            ...
            set_agent_and_clean(code, r'ا.?م.?ض.?ا.?ء?\s*(ماريهان|مريهان|مايهان)')

        # 5) فاطمه محمود / سعداوي
        if confirmation_agent is None and any(
            word in normalized for word in ['فاطمهمحمود', 'فاطمهمسعداوي', 'فاطمهسعداوي']
        ):
            code = '004' if agent_id == '250610' else '018'
            set_agent_and_clean(code, r'ا.?م.?ض.?ا.?ء?\s*فاطم[هة]\s*(محمود|سعداو[يى])')
        ...
        if confirmation_agent is None and (
            ...
            set_agent_and_clean(code, r'ا.?م.?ض.?ا.?ء?\s*مرو[هة]\s*مصطف[يى]')

        # 7) ساره احمد
        if confirmation_agent is None and re.search(r'سار[هةا]*\s*احمد', normalized):
            set_agent_and_clean('017', r'ا.?م.?ض.?ا.?ء?\s*سار[هةا]*\s*احمد')
